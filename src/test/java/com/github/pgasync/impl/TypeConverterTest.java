...
public class TypeConverterTest extends ConnectedTestBase {
    ...
    public void shouldConvertInt8ToBigInteger() {
        assertEquals(new BigInteger("9223372036854775807"), query("select 9223372036854775807::INT8").get(0)
                .getBigInteger(0));
    }
    ...
    public void shouldConvertDateToDate() {
        assertEquals(new Date(millis(2014, 1, 31, 0, 0, 0, 0)),
                query("select '2014-01-31'::DATE").get(0).getDate(0));
    }

    @Test
    public void shouldConvertDateToDateWithName() {
        assertEquals(new Date(millis(2014, 2, 21, 0, 0, 0, 0)), query("select '2014-02-21'::DATE as D").get(0)
                .getDate("D"));
    }

    @Test
    public void shouldConvertTimeToTime() {
        assertEquals(new Time(millis(0, 0, 0, 10, 15, 31, 123)), query("select '10:15:31.123'::TIME").get(0)
                .getTime(0));
    }

    @Test
    public void shouldConvertZonedTimeToTime() {
        assertEquals(new Time(millis(0, 0, 0, 23, 59, 59, 999)), query("select '23:59:59.999Z'::TIMETZ as zoned")
                .get(0).getTime("zoned"));
    }
    ...
    public void shouldConvertByteAToBytesWithName() {
        assertArrayEquals(new byte[] { 0x41, 0x41 }, query("select $1::BYTEA as bytes", asList("AA")).get(0)
                .getBytes("bytes"));
    }
